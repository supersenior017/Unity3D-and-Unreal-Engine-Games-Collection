...
public class Wall : MonoBehaviour
{
    ...
    void Update()
    {
        if (Defensive_Cooldown_Image.GetComponent<Image>().fillAmount == 1 && PauseObject.GetComponent<Pause>().pause == false && CooldownObject.GetComponent<Cooldown>().DefensiveWallIsActivated == true) //Cooldown fertig? && Pause deaktivert?
        {
            if (Input.GetKeyDown("2") && Wall_Active == false)
            {
                IsResetted = false;
                ...
                gameObject.GetComponent<Slowly>().enabled = false;
        ...
        if (current_time >= wall_time && IsResetted == false)
        {
            ...
            gameObject.GetComponent<Slowly>().enabled = true;
